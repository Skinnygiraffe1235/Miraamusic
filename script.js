...
const bookSocialLinks = document.querySelector('.tab-view[data-tab-view="about"] .social-links');
const alterEgoSocialLinks = document.querySelector('.tab-view[data-tab-view="alterego"] .social-links');
if (bookSocialLinks && alterEgoSocialLinks) {
  alterEgoSocialLinks.replaceChildren(...[...bookSocialLinks.children].map(link => link.cloneNode(true)));
  alterEgoSocialLinks.setAttribute('aria-label', bookSocialLinks.getAttribute('aria-label'));
}
...
const tracks = [
  ...
  { title: 'Miraamusic - ENAN', artist: 'Mahdi · Full Pop Rock Production', type: 'local', url: 'enan.mp3', accent: '#e1527e' },
